...
helm repo add qlik https://qlik.bintray.com/stable 2>&1 | tee -a $LOG_FILE
if [ $? -eq 0 ]; then
...
echo "   [OK]   Update helm repos" 2>&1 | tee -a $LOG_FILE
helm install --name qliksense-init qlik/qliksense-init 2>&1 | tee -a $LOG_FILE
if [ $? -eq 0 ]; then
    echo "   [OK]   Custom resource definitions used by dynamic engines installed" 2>&1 | tee -a $LOG_FILE
else
    echo "   [ERROR]   while installing custom resource definition. Exit Program" 2>&1 | tee -a $LOG_FILE
    error_handling
fi


helm install -n qliksense qlik/qliksense -f ./values.yaml 2>&1 | tee -a $LOG_FILE
if [ $? -eq 0 ]; then
